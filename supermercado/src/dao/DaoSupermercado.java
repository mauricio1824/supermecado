...
public class DaoSupermercado {
    ...
    public static void main(String[] args) {
     ...
     public static boolean alterar(Supermercado objeto) {
        String sql = "UPDATE supermercado SET nome_fantasia = ?, razao_social = ?, fundacao = ?, n_funcionarios = ?, valor_bolsa = ? WHERE codigo=?";
        try {
            PreparedStatement ps = conexao.Conexao.getConexao().prepareStatement(sql);
            ps.setString(1, objeto.getNome_fantasia());
            ps.setString(2, objeto.getRazao_social());
     ...
     public static List<Supermercado> consultar() {
        ...
        String sql = "SELECT codigo, nome_fantasia, razao_social, fundacao, n_funcionarios, valor_bolsa FROM supermercado";
        ...
        try {
            ...
            while (rs.next()) {
                ...
                objeto.setN_funcionarios(rs.getInt("n_funcionarios"));
     ...
     public static Supermercado consultar(int primaryKey) {
        //editar o SQL conforme a entidade
        String sql = "SELECT codigo, nome_fantasia, razao_social, fundacao, n_funcionarios, valor_bolsa FROM supermercado WHERE codigo=?";
        PreparedStatement ps;
        try {
            ps = conexao.Conexao.getConexao().prepareStatement(sql);
